...
// The artist page's "Albums" / "Singles & EPs" carousels only hold the first
// ten releases; the shelf's "More" button points at the full discography
// (browseId MPAD…), a grid of every release plus an optional continuation.
// youtubei.js has no parser for that page, so walk the raw JSON.
//
// Cached for a day: the walk is up to six sequential /browse round-trips and
// is what makes a cold artist page slow, while a discography changes on the
// timescale of release days, not visits. The artist cache upstream only lives
// 30 minutes, so without this every revisit past that pays the walk again.
const discoCache = new Map(); // "MPAD…:params" -> { releases, exp }

async function getDiscography(yt, endpoint) {
  const payload = endpoint?.payload;
  if (!payload?.browseId?.startsWith("MPAD")) return null;
  const cacheKey = `${payload.browseId}:${payload.params || ""}`;
  const hit = discoCache.get(cacheKey);
  if (hit && hit.exp > Date.now()) return hit.releases;
  const releases = [];
  const seen = new Set();
  const collect = (o) => {
    ...
    if (r) {
      const token = r.navigationEndpoint?.browseEndpoint?.browseId || "";
      if (token.startsWith("MPRE") && !seen.has(token)) {
        seen.add(token);
        ...
        releases.push({
          token,
          title: r.title?.runs?.[0]?.text || "Unknown Album",
  ...
  const findContinuation = (o) => {
    if (Array.isArray(o)) { for (const x of o) { const c = findContinuation(x); if (c) return c; } return ""; }
    if (!o || typeof o !== "object") return "";
    const c = o.continuations?.[0]?.nextContinuationData?.continuation;
    if (c) return c;
    for (const k of Object.keys(o)) { const r = findContinuation(o[k]); if (r) return r; }
    return "";
  };
  let body = { browseId: payload.browseId, params: payload.params, client: "YTMUSIC" };
  for (let page = 0; page < 6; page++) {
    const res = await yt.actions.execute("/browse", body);
    collect(res?.data);
    const cont = findContinuation(res?.data);
    if (!cont) break;
    body = { continuation: cont, client: "YTMUSIC" };
  }
  if (!releases.length) return null; // a miss is not worth remembering
  discoCache.set(cacheKey, { releases, exp: Date.now() + 24 * 60 * 60 * 1000 });
  return releases;
}
...
export async function getArtist(channelId) {
  ...
  const full = []; // full-discography fetches, resolved after the fast pass
  for (const section of artist?.sections ?? []) {
    ...
    if (!out.songs.length && (title.includes("song") || title.includes("top"))) {
      ...
      out.albums = contents.map(mapRelease).filter(Boolean);
      full.push(getDiscography(yt, section.header?.more_content?.endpoint)
        .then((all) => { if (all) out.albums = all; }).catch(() => {}));
    } else if (title.includes("single") || title.includes("ep")) {
      out.singles = contents.map(mapRelease).filter(Boolean);
      full.push(getDiscography(yt, section.header?.more_content?.endpoint)
        .then((all) => { if (all) out.singles = all; }).catch(() => {}));
...
// yt-dlp negotiates with one of YouTube's player clients, and which one still
// works changes without warning. When a client's URLs start coming back 403,
// asking a different client is usually all it takes — so the resolver takes a
// client name and the route works down this ladder before giving up.
export const STREAM_CLIENTS = config.streamClients;

// The argv is built separately from the spawn so it can be tested without
// running anything: whether -4 and player_client actually reach yt-dlp is
// exactly the kind of thing that breaks silently and reads fine in review.
export function buildStreamArgs(videoId, quality = "high", { ipv4 = false, client = "" } = {}) {
  const format = FORMATS[quality] ?? FORMATS.high;
  return [
    // Only this resolve needs pinning: it is the one place a googlevideo URL
    // is handed to a different HTTP client (our fetch) instead of being
    // downloaded by yt-dlp itself, so the two legs must leave by one address.
    ...(config.forceIpv4 || ipv4 ? ["-4"] : []),
    ...(client ? ["--extractor-args", `youtube:player_client=${client}`] : []),
    "-f", format,
    // two lines out: the URL, then the headers it needs, as JSON. Lighter
    // than -J, which dumps the entire info dict.
    "--print", "%(url)s",
...
export function getStreamSource(videoId, quality = "high", opts = {}) {
  if (!/^[\w-]{6,20}$/.test(videoId)) {
    return Promise.reject(new Error("invalid video id"));
  }
  return new Promise((resolve, reject) => {
    execFile(
      findYtdlp(),
      buildStreamArgs(videoId, quality, opts),
...
// Which yt-dlp is actually in this image? The install layer is cached by
// Docker, so a rebuilt image can still carry a months-old extractor — and a
// stale extractor produces URLs YouTube then refuses. Logged at boot so that
// question is answered before anyone has to ask it.
let ytdlpVersionCache = null;
export function ytdlpVersion() {
  if (ytdlpVersionCache) return Promise.resolve(ytdlpVersionCache);
  return new Promise((resolve) => {
    execFile(findYtdlp(), ["--version"], { timeout: 15000, windowsHide: true }, (err, stdout) => {
      ytdlpVersionCache = err ? "unknown" : String(stdout).trim().split(/\r?\n/)[0];
      resolve(ytdlpVersionCache);
    });
  });
}
