...
export function addSongs(jam, rawSongs, byName = "") {
  ...
  jam.queue.splice(userQueueEnd(jam), 0, ...songs);
  if (wasEmpty) {
...
export function markEnded(jam, i, deviceId = "") {
  ...
  } else if (jam.extending) {
    // more songs are on their way — don't declare the end; whoever awaited
    // the refill advances once it lands
    return false;
  } else {
    jam.playing = false;
    jam.pos = 0;
