...
export const config = {
  ...
  // A googlevideo URL is minted for the exact address that asked for it, so
  // yt-dlp's resolve and our fetch of the result must leave by the same one.
  // On a dual-stack line they can disagree — yt-dlp goes out over IPv4 while
  // Node's fetch races Happy Eyeballs and wins on IPv6 — and Google answers
  // the mismatch with a 403. Set FORCE_IPV4=1 to pin both legs to IPv4.
  forceIpv4: ["1", "true", "yes"].includes(String(process.env.FORCE_IPV4 || "").toLowerCase()),
  // Fallback ladder for stream resolution. yt-dlp picks a YouTube player
  // client on its own (ANDROID_VR at time of writing) and when YouTube
  // stops serving that one, another usually still works. Tried in order,
  // only after a refusal, and the winner is reused for the rest of the
  // process.
  //
  // These two were measured, not guessed — every other candidate failed:
  // web/web_safari/ios/tv return no usable audio formats at all, mweb
  // resolved but its URLs were refused, web_embedded passed once and was
  // refused the next time, and tv_embedded silently falls back
  // to ANDROID_VR, which is the client we are trying to get away from.
  // Re-probe before changing these; a ladder of broken clients looks like a
  // feature and does nothing.
  streamClients: (process.env.STREAM_CLIENTS || "web_music,android")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean),
