// Jam sessions: shared real-time listening. Everything lives in memory —
// jams are ephemeral by design (a server restart ends the party, like
// Spotify's Jam). The server clock is the single source of truth for
// playback: state is { index, playing, pos, at } where `pos` is seconds into
// the track as of server-time `at`, so any client can derive the current
// position without a steady stream of progress reports.
//
...
// ---- lifecycle ----
// speakerId names the one device whose <audio> actually plays — everyone
// else is a synchronized remote control. It starts as the creating device.
export function createJam(user, seed = {}, speakerId = "") {
  leaveJam(user.id); // one jam per user
  ...
  const jam = {
    code: makeCode(),
    hostId: user.id,
    speakerId: String(speakerId || "").slice(0, 64),
    created: now(),
    lastActive: now(),
...
// the speaker device's audio finished the track. Only the speaker may report
// (remotes have no audio), and only a report for the current index advances.
